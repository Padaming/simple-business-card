...
export function ExampleCards({ cards }: ExampleCardsProps) {
  if (cards.length === 0) {
    return (
      <div className="w-full flex flex-col items-center justify-center p-12 bg-white rounded-2xl border border-dashed border-gray-300">
        <p className="text-lg text-gray-600 mb-2">目前還沒有名片</p>
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 w-full" data-testid="example-cards-container">
          ...
          <Link href={`/cards/${card.slug}`} className="group block h-full no-underline">
            <CardUI className="h-full border-0 shadow-sm hover:shadow-md hover:bg-[#D9CD90] bg-white transition-all duration-500">
              <CardContent className="p-6 flex flex-col items-center text-center h-full">
                ...
                <p className="text-sm text-gray-500 group-hover:text-white/80 font-medium line-clamp-1 w-full transition-colors duration-500">{card.title}</p>
              </CardContent>
            </CardUI>
          </Link>
        </div>
